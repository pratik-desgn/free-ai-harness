...
import { bestSearchResults, builtInTools, parseSearchResults } from "../src/tools.js";
...
test("search aggregation prefers dated numeric evidence over generic live links", () => {
  const year = new Date().getUTCFullYear();
  const generic = "1. Live Market\nhttps://example.com/live\nSee current market updates";
  const dated = `1. Market closes lower\nhttps://example.com/report\nToday ${year}-08-10: index 2,650.09 fell 4.05 points (-0.15%); turnover Rs. 3.5 billion`;
  assert.equal(bestSearchResults([generic, dated], "market update today"), dated);
});
