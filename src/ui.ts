...
async function poll(id){const response=await fetch('/v1/runs/'+id),run=await response.json();status.innerHTML='<div class="event"><strong>'+escapeHtml(run.status)+' · step '+Number(run.step)+'</strong></div>'+run.events.map(e=>'<div class="event">'+escapeHtml(e.message)+'</div>').join('');if(run.status==='completed'){result.className='result';result.textContent=run.result;const feedback=document.createElement('div');feedback.innerHTML='<button class="ghost" data-rating="1">Useful</button> <button class="ghost" data-rating="-1">Needs work</button>';feedback.querySelectorAll('button').forEach(item=>item.addEventListener('click',async()=>{await fetch('/v1/runs/'+id+'/feedback',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({rating:Number(item.dataset.rating)})});feedback.textContent='Feedback saved';}));result.appendChild(feedback);button.disabled=false;await loadRuns();return;}if(run.status==='failed'||run.status==='cancelled'){result.className='result';result.textContent=run.error||'Workflow cancelled';button.disabled=false;await loadRuns();return;}setTimeout(()=>poll(id),1000);}
